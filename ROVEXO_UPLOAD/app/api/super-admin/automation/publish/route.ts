...
const bodySchema = z.object({ approvalId: z.string().optional(), mfaVerified: z.boolean().optional() }).passthrough();
...
export async function POST(request: Request) {
  ...
  try {
    const result = await executeAutomationAction("publish", auth.user.id, { ...parsed.data, mfaVerified: true });
