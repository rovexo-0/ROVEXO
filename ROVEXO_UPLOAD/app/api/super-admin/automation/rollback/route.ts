...
const bodySchema = z.object({ versionId: z.string().optional(), mfaVerified: z.boolean().optional() }).passthrough();
...
export async function POST(request: Request) {
  ...
  try {
    const result = await executeAutomationAction("rollback", auth.user.id, { ...parsed.data, mfaVerified: true });
