...
const bodySchema = z.object({ releaseId: z.string().optional(), mfaVerified: z.boolean().optional() });
...
export async function POST(request: Request) {
  ...
  try {
    const result = await executeDeploymentAction("validate", auth.user.id, { ...parsed.data, mfaVerified: true });
