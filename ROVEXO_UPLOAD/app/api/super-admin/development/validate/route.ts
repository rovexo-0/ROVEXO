...
export async function POST(request: Request) {
  ...
  try {
    const result = await executeDevelopmentAction("validate", auth.user.id, { ...body, mfaVerified: true });
    const development = await getDevelopmentSnapshot();
    return NextResponse.json({ ok: true, ...result, development });
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "validate failed" }, { status: 400 });
  }
}
