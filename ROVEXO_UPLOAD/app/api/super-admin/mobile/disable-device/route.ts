...
export async function POST(request: Request) {
  ...
  try {
    const result = await executeMobileCcAction("disable-device", auth.user.id, { ...parsed.data, mfaVerified: true });
    const mobileControlCenter = await getMobileCcSnapshot("devices");
    return NextResponse.json({ ok: true, ...result, mobileControlCenter });
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "Disable device failed" }, { status: 400 });
  }
}
