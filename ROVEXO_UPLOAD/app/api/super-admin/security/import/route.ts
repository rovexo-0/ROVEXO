...
const bodySchema = z.object({ data: z.string().min(1), mfaVerified: z.boolean().optional() });
...
export async function POST(request: Request) {
  ...
  try {
    const result = await executeSocAction("import", auth.user.id, { ...parsed.data, mfaVerified: true });
