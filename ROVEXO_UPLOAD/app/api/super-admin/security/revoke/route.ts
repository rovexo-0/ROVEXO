...
const bodySchema = z.object({ sessionId: z.string().optional(), deviceId: z.string().optional(), mfaVerified: z.boolean().optional() });
...
export async function POST(request: Request) {
  ...
  try {
    const result = await executeSocAction("revoke", auth.user.id, { ...parsed.data, mfaVerified: true });
    const securityOperationsCenter = await getSocSnapshot("sessions");
