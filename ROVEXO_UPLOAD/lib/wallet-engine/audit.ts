...
export async function auditWalletEngineAction(input: {
  actorId: string;
  module: string;
  component?: string;
  action: string;
  ...
  await auditSuperAdminAction({
    ...
    action: "wallet_engine.change",
    resourceType: "wallet_engine",
...
export function createWalletEngineAuditEntry(input: {
