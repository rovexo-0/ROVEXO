-- Launch feature tables referenced in application code (idempotent)
...
drop policy if exists "saved_search_notification_log_select_own" on public.saved_search_notification_log;
create policy "saved_search_notification_log_select_own"
  on public.saved_search_notification_log for select
  using (
    exists (
      select 1
      from public.saved_searches ss
      where ss.id = saved_search_id and ss.user_id = auth.uid()
    )
  );
