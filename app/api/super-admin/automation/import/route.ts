...
const bodySchema = z.object({ data: z.string().optional(), mfaVerified: z.boolean().optional() }).passthrough();
...
export async function POST(request: Request) {
  ...
  try {
    const result = await executeAutomationAction("import", auth.user.id, { ...parsed.data, mfaVerified: true });
