...
const bodySchema = z.object({ releaseId: z.string().min(1), mfaVerified: z.boolean().optional() });
...
export async function POST(request: Request) {
  ...
  try {
    const result = await executeDeploymentAction("approve", auth.user.id, { ...parsed.data, mfaVerified: true });
