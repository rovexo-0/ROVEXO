...
const bodySchema = z.object({ version: z.string().optional(), mfaVerified: z.boolean().optional() }).passthrough();
...
export async function POST(request: Request) {
  ...
  try {
    const result = await executeDeploymentAction("build", auth.user.id, { ...parsed.data, mfaVerified: true });
