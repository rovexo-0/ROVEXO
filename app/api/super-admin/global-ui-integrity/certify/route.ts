...
export async function POST(request: Request) {
  ...
  try {
    const result = await executeGlobalUiIntegrityAction("certify", auth.user.id, { ...parsed.data, mfaVerified: true });
    const globalUiIntegrity = await getGlobalUiIntegritySnapshot("certification");
