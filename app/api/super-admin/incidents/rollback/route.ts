...
export async function POST(request: Request) {
  ...
  try {
    const result = await executeIncidentAction("rollback", auth.user.id, { ...parsed.data, mfaVerified: true });
    const incidentResponseCenter = await getIncidentSnapshot("playbooks");
