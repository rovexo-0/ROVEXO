...
export async function POST() {
  ...
  try {
    const result = await executeOmegaAction("quick-scan", auth.user.id, { mfaVerified: true });
    const omega = await getOmegaSnapshot();
    return NextResponse.json({ ok: true, ...result, omega });
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "Quick scan failed" }, { status: 400 });
  }
}
