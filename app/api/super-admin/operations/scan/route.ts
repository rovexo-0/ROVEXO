...
function scanErrorMessage(error: unknown): string {
  ...
  return error instanceof Error ? error.message : "Operations scan failed.";
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return enterpriseErrorResponse(scanErrorMessage(error), {
      request,
      startedAt,
      status: 500,
      diagnostics: {
        route: "/api/super-admin/operations/scan",
        category: isDatabasePermissionError(error) ? "database_permission" : "scan_failure",
      },
    });
  }
}
