...
const bodySchema = z.object({ ip: z.string().min(1), mfaVerified: z.boolean().optional() });
...
export async function POST(request: Request) {
  ...
  try {
    const result = await executeSocAction("unblock", auth.user.id, { ...parsed.data, mfaVerified: true });
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : "Unblock failed" }, { status: 400 });
  }
}
