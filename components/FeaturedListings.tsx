...
export default function FeaturedListings({ listings = [] }: FeaturedListingsProps) {
  return (
    <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8 lg:py-14" aria-labelledby="featured-heading">
      <div className="mb-8 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h2 id="featured-heading" className="text-2xl font-extrabold tracking-tight text-gray-900 sm:text-3xl">
            Featured listings
          </h2>
          <p className="mt-1 text-sm text-gray-500 sm:text-base">
            Hand-picked deals from top-rated sellers
          </p>
        </div>
        <Link href="/featured" className="text-sm font-semibold text-[#2563eb] hover:opacity-80">
          View all →
        </Link>
      </div>
      ...
      {listings.length > 0 ? (
        ...
        <div className="rounded-2xl border border-dashed border-gray-200 bg-gray-50 py-16 text-center">
          <p className="text-sm text-gray-500">Featured listings will appear here.</p>
        </div>
      )}
    </section>
  );
