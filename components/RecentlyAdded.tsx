  ...
  return (
    <article className="group flex flex-col overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-xl">
      <Link href={`/listing/${listing.slug}`} className="relative block aspect-square overflow-hidden bg-gray-50">
        ...
        />
        <span className="absolute left-3 top-3 rounded-full bg-green-500 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide text-white">
          New
        </span>
      </Link>
      <div className="flex flex-1 flex-col gap-1.5 p-4">
        ...
        <div className="mt-auto flex items-center justify-between pt-1 text-xs">
          ...
          <time className="text-green-600" dateTime={listing.addedAt}>
            {listing.addedAt}
          </time>
        </div>
        <button
...
export default function RecentlyAdded({ listings = [] }: RecentlyAddedProps) {
  return (
    <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8 lg:py-14" aria-labelledby="recent-heading">
      <div className="mb-8 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h2 id="recent-heading" className="text-2xl font-extrabold tracking-tight text-gray-900 sm:text-3xl">
            Recently added
          </h2>
          <p className="mt-1 text-sm text-gray-500 sm:text-base">
            Fresh listings from sellers across ROVEXO
          </p>
        </div>
        <Link href="/recent" className="text-sm font-semibold text-[#2563eb] hover:opacity-80">
          All new items →
        </Link>
      </div>
      ...
      {listings.length > 0 ? (
        ...
        <div className="rounded-2xl border border-dashed border-gray-200 bg-gray-50 py-16 text-center">
          <p className="text-sm text-gray-500">Recent listings will appear here.</p>
        </div>
      )}
    </section>
  );
