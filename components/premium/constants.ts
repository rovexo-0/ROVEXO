...
export const PREMIUM_CATEGORIES: readonly PremiumCategory[] = ROVEXO_HOME_CATEGORY_RAIL.map(
...
export const PREMIUM_BUSINESS_FALLBACK: PremiumBusiness[] = [
  {
    id: "biz-1",
    name: "TechVault Pro",
    slug: "techvault-pro",
  ...
  },
  {
    ...
    logoUrl: getCategoryPremiumPngSrc("luxury"),
    verified: true,
    category: "Luxury",
    listingCount: 64,
...
export function getCategoryIconSrc(icon: string): string {
  return getCategoryPremiumPngSrc(resolveCategoryPremiumIcon(icon));
}
...
export function deriveBusinessesFromProducts(products: Product[]): PremiumBusiness[] {
  const seen = new Set<string>();
  const businesses: PremiumBusiness[] = [];
  ...
  for (const product of products) {
    ...
    businesses.push({
      ...
      slug: product.slug,
      logoUrl: product.sellerAvatar || product.imageUrl || getCategoryPremiumPngSrc("business"),
  ...
  return businesses.length > 0 ? businesses : PREMIUM_BUSINESS_FALLBACK;
}
