...
export function SellerBalanceCard() {
  ...
  return (
    <SellerSection id="seller-balance" title="Payments" href="/seller/wallet">
      <div className="seller-card">
        <div className="seller-metric-grid">
          ...
          <div className="seller-metric">
            <p className="seller-metric__value">€{wallet.pendingBalance.toLocaleString()}</p>
            <p className="seller-metric__label">Pending balance</p>
          </div>
        </div>
      </div>
      {wallet.transactions.slice(0, 3).map((transaction) => (
        <Link key={transaction.id} href={`/seller/wallet/transactions/${transaction.id}`} className="seller-list-row">
          <div className="min-w-0">
            <p className="seller-list-row__title">{transaction.productTitle}</p>
            <p className="seller-list-row__meta">€{transaction.amount.toLocaleString()} · {transaction.status}</p>
          </div>
        </Link>
      ))}
    </SellerSection>
