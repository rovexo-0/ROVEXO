...
import { Check, Loader2, ShoppingBag, ShoppingCart, Tag } from "lucide-react";
    ...
    >
      <span className="pd-v1__action-btn-inner">
        <span className="pd-v1__action-icon" aria-hidden>
          {showSpinner ? (
            <Loader2 className="pd-v1__action-spinner" strokeWidth={2.25} />
          ) : showCheck ? (
        ...
        if (button.id === "buy_now") {
          return (
            <ActionButton
              ...
              icon={<ShoppingBag strokeWidth={2.25} aria-hidden />}
              variant="primary"
