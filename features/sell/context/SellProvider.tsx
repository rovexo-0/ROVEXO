...
/** Draft fields — stable when only publish progress ticks (P7). */
export type SellDraftContextValue = {
...
/** Photos array only — stable when title/price/category update (P7). */
export type SellPhotosContextValue = {
  photos: SellListingDraft["photos"];
  showValidation: boolean;
};

/** High-frequency publish/upload progress — isolated from form blocks (P7). */
export type SellPublishProgressContextValue = {
  isPublishing: boolean;
  publishPhase: PublishPhase;
  uploadProgress: number;
};

/** Low-frequency publish outcome — isolated from progress ticks (P7). */
export type SellPublishOutcomeContextValue = {
  view: SellView;
  formError: string | null;
  publishSuccess: PublishSuccessPayload | null;
};

/** Stable callbacks + pending text refs (P7). */
export type SellActionsContextValue = {
...
export type SellContextValue = SellDraftContextValue &
  SellPublishProgressContextValue &
  SellPublishOutcomeContextValue &
  SellActionsContextValue;
...
const SellPhotosContext = createContext<SellPhotosContextValue | null>(null);
const SellPublishProgressContext = createContext<SellPublishProgressContextValue | null>(null);
const SellPublishOutcomeContext = createContext<SellPublishOutcomeContextValue | null>(null);
const SellActionsContext = createContext<SellActionsContextValue | null>(null);
...
function useSellFormInternal(options: SellProviderOptions = {}): SellContextValue {
  ...
  /** Client-only session id — never allocate during SSR (hydration-safe). */
  const photoSessionOwnerIdRef = useRef<string>("");
  const ensurePhotoSessionOwnerId = useCallback((): string => {
    if (!photoSessionOwnerIdRef.current) {
      photoSessionOwnerIdRef.current = safeRandomUUID();
    }
    return photoSessionOwnerIdRef.current;
  }, []);
  ...
  useEffect(() => {
    prepareSellCameraEntry(ensurePhotoSessionOwnerId());
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const ownerId = ensurePhotoSessionOwnerId();
      setDraft((current) => {
        ...
        if (merged.photos.length > 0) {
          void resumeSellDraftPhotosIntoSession(ownerId, merged.photos);
        }
        return merged;
      });
    })();
  ...
  }, [editListingId, ensurePhotoSessionOwnerId, freshSession, initialDraft, restoreDraft]);

  useEffect(() => {
    if (!initialDraft?.photos.length) return;
    void resumeSellDraftPhotosIntoSession(ensurePhotoSessionOwnerId(), initialDraft.photos);
  }, [ensurePhotoSessionOwnerId, initialDraft]);
  ...
  const restoreLocalDraft = useCallback(async () => {
    ...
    if (restored.photos.length > 0) {
      void resumeSellDraftPhotosIntoSession(ensurePhotoSessionOwnerId(), restored.photos);
    } else {
      prepareSellCameraEntry(ensurePhotoSessionOwnerId());
    }
  }, [ensurePhotoSessionOwnerId]);
  ...
  useEffect(() => {
    ...
    const isDraftEditAutosave = Boolean(editListingId && editListingStatus === "draft");
    if (!isCreateAutosave && !isDraftEditAutosave) return;
    // P10.1 — empty / post-publish reset shell must not schedule autosave.
    if (!isMeaningfulDraft(draft, draft.photos.length)) return;
  ...
  useEffect(() => {
    ...
    const persistOnHide = () => {
      // P10.1 — empty post-publish shell must not sync or POST on hide.
      if (!isMeaningfulDraft(draftRef.current, draftRef.current.photos.length)) return;
  ...
  }, [ensurePhotoSessionOwnerId, pushToast, refreshSmartDescription, resolvePhotoEntrySource, uploadPhoto]);
  ...
  const removePhoto = useCallback(async (id: string) => {
    ...
    try {
      ...
      removeSellPhotoViaCanonicalEntry(ensurePhotoSessionOwnerId(), id);
  ...
  const replacePhoto = useCallback((id: string, file: File) => {
    ...
    void (async () => {
      const result = await intakeSellPhotoFromCanonicalEntry({
        ownerId: ensurePhotoSessionOwnerId(),
  ...
  const reorderPhotos = useCallback((fromIndex: number, toIndex: number) => {
    ...
    reorderSellPhotosViaCanonicalEntry(ensurePhotoSessionOwnerId(), fromIndex, toIndex);
  }, [ensurePhotoSessionOwnerId]);
  ...
  const setMainPhoto = useCallback((id: string) => {
    setDraft((current) => {
      ...
      reorderSellPhotosViaCanonicalEntry(ensurePhotoSessionOwnerId(), index, 0);
      return { ...current, photos };
    });
  }, [ensurePhotoSessionOwnerId]);
  ...
  const publishListing = useCallback(async () => {
    ...
    try {
      ...
      // Clear validation BEFORE empty draft + pending-text bump so consumers that
      // still call full-draft validators never see showValidation=true with a
      // reset photos array in the same turn (P9.3.1).
      setShowValidation(false);
      setFormError(null);
...
export function SellProvider({ children, ...options }: SellProviderProps) {
  ...
  const draftSlice = useMemo<SellDraftContextValue>(
    () => ({
      draft: value.draft,
      showValidation: value.showValidation,
      descriptionIsAutoGenerated: value.descriptionIsAutoGenerated,
      editListingId: value.editListingId,
      editListingSlug: value.editListingSlug,
      listingMode: value.listingMode,
    }),
    [
      value.draft,
      value.showValidation,
      value.descriptionIsAutoGenerated,
      value.editListingId,
      value.editListingSlug,
      value.listingMode,
    ],
  );

  const photosSlice = useMemo<SellPhotosContextValue>(
    () => ({
      photos: value.draft.photos,
      showValidation: value.showValidation,
    }),
    [value.draft.photos, value.showValidation],
  );

  const publishProgress = useMemo<SellPublishProgressContextValue>(
    () => ({
      isPublishing: value.isPublishing,
      publishPhase: value.publishPhase,
      uploadProgress: value.uploadProgress,
    }),
    [value.isPublishing, value.publishPhase, value.uploadProgress],
  );

  const publishOutcome = useMemo<SellPublishOutcomeContextValue>(
    () => ({
      view: value.view,
      formError: value.formError,
      publishSuccess: value.publishSuccess,
    }),
    [value.view, value.formError, value.publishSuccess],
  );

  const actions = useMemo<SellActionsContextValue>(
    () => ({
      pendingTitleRef: value.pendingTitleRef,
      pendingDescriptionRef: value.pendingDescriptionRef,
      flushTitleCommitRef: value.flushTitleCommitRef,
      flushDescriptionCommitRef: value.flushDescriptionCommitRef,
      syncTitleToDraft: value.syncTitleToDraft,
      syncDescriptionToDraft: value.syncDescriptionToDraft,
      addPhotos: value.addPhotos,
      removePhoto: value.removePhoto,
      replacePhoto: value.replacePhoto,
      reorderPhotos: value.reorderPhotos,
      setMainPhoto: value.setMainPhoto,
      retryPhotoUpload: value.retryPhotoUpload,
      updateDraft: value.updateDraft,
      setCategoryPath: value.setCategoryPath,
      publishListing: value.publishListing,
      restoreLocalDraft: value.restoreLocalDraft,
      discardRecoveryDraft: value.discardRecoveryDraft,
      markDescriptionManuallyEdited: value.markDescriptionManuallyEdited,
      resetForAnotherListing: value.resetForAnotherListing,
      getIsDirty: value.getIsDirty,
    }),
    [
      value.pendingTitleRef,
      value.pendingDescriptionRef,
      value.flushTitleCommitRef,
      value.flushDescriptionCommitRef,
      value.syncTitleToDraft,
      value.syncDescriptionToDraft,
      value.addPhotos,
      value.removePhoto,
      value.replacePhoto,
      value.reorderPhotos,
      value.setMainPhoto,
      value.retryPhotoUpload,
      value.updateDraft,
      value.setCategoryPath,
      value.publishListing,
      value.restoreLocalDraft,
      value.discardRecoveryDraft,
      value.markDescriptionManuallyEdited,
      value.resetForAnotherListing,
      value.getIsDirty,
    ],
  );

  const composed = useMemo<SellContextValue>(
    () => ({ ...draftSlice, ...publishProgress, ...publishOutcome, ...actions }),
    [draftSlice, publishProgress, publishOutcome, actions],
  );

  return (
    <SellActionsContext.Provider value={actions}>
      <SellPublishProgressContext.Provider value={publishProgress}>
        <SellPublishOutcomeContext.Provider value={publishOutcome}>
          <SellDraftContext.Provider value={draftSlice}>
            <SellPhotosContext.Provider value={photosSlice}>
              <SellContext.Provider value={composed}>{children}</SellContext.Provider>
            </SellPhotosContext.Provider>
          </SellDraftContext.Provider>
        </SellPublishOutcomeContext.Provider>
      </SellPublishProgressContext.Provider>
    </SellActionsContext.Provider>
  );
}
...
export function useSellDraft(): SellDraftContextValue {
  const context = useContext(SellDraftContext);
  if (!context) {
    throw new Error("useSellDraft must be used within SellProvider");
  }
  return context;
}

export function useSellPhotos(): SellPhotosContextValue {
  const context = useContext(SellPhotosContext);
  if (!context) {
    throw new Error("useSellPhotos must be used within SellProvider");
  }
  return context;
}

export function useSellPublishProgress(): SellPublishProgressContextValue {
  const context = useContext(SellPublishProgressContext);
  if (!context) {
    throw new Error("useSellPublishProgress must be used within SellProvider");
  }
  return context;
}

export function useSellPublishOutcome(): SellPublishOutcomeContextValue {
  const context = useContext(SellPublishOutcomeContext);
  if (!context) {
    throw new Error("useSellPublishOutcome must be used within SellProvider");
  }
  return context;
}

export function useSellActions(): SellActionsContextValue {
  const context = useContext(SellActionsContext);
  if (!context) {
    throw new Error("useSellActions must be used within SellProvider");
  }
  return context;
}
