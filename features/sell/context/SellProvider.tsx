...
function useSellFormInternal(options: SellProviderOptions = {}): SellContextValue {
  ...
  const addPhotos = useCallback(async (files: FileList | File[]) => {
    ...
    const accepted: Array<{
      photo: SellPhoto;
      draftMetadata: PhotoMetadataEntry;
    }> = [];
    ...
    for (const file of selected) {
      try {
        ...
        accepted.push({
          photo: result.photo,
          draftMetadata: result.draftMetadata,
        });
      } catch (error) {
    ...
    if (accepted.length > 0) {
      const added = accepted.map((item) => item.photo);
      ...
      for (const item of accepted) {
        ...
        void (async () => {
          const colour = photo.file
            ? await detectColourFromImageFile(photo.file)
            : null;
          photoMetadataRef.current = [
            ...photoMetadataRef.current.filter((entry) => entry.id !== photo.id),
            {
              ...item.draftMetadata,
              dominantColour: colour ?? item.draftMetadata.dominantColour,
            },
          ];
          refreshSmartDescription();
