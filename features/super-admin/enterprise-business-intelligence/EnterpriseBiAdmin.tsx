...
export function EnterpriseBiAdmin({ initialSnapshot, defaultTab = "dashboard" }: EnterpriseBiAdminProps) {
  ...
  return (
    <div className="ebi-admin">
      <header className="ebi-admin__header">
        <div>
          <p className="ebi-admin__eyebrow">Enterprise Business Intelligence Center</p>
          <h2 className="ebi-admin__title">Executive Analytics Platform</h2>
          <p className="ebi-admin__desc">KPI engine, marketplace analytics, AI forecasting, and executive reports for data-driven decisions.</p>
        </div>
        <div className="ebi-admin__scores">
          <div className="ebi-score ebi-score--revenue"><span>Revenue</span><strong>{formatCurrency(snapshot.dashboard.revenue)}</strong></div>
          <div className="ebi-score ebi-score--growth"><span>Growth</span><strong>+{snapshot.dashboard.marketplaceGrowth}%</strong></div>
          <div className="ebi-score ebi-score--health"><span>Health</span><strong>{snapshot.dashboard.platformHealth}%</strong></div>
        </div>
      </header>

      <div className="ebi-admin__actions">
        ...
        <Link href="/super-admin/analytics-engine" className="ebi-link">Analytics Engine</Link>
        <Link href="/super-admin/ai" className="ebi-link">AI Operating System</Link>
      </div>

      {message && <p className="ebi-admin__message">{message}</p>}
      {snapshot.pendingPublish && <p className="ebi-admin__banner">Pending publish — draft differs from live.</p>}

      <nav className="ebi-tabs" aria-label="BI sections">
        {NAV_ROUTES.map((route) => (
          <Link key={route.id} href={route.href} className={cn("ebi-tab", (activeTab === route.id || (activeTab === "dashboard" && route.id === "dashboard")) && "ebi-tab--active")}>
            {route.label}
          </Link>
        ))}
      </nav>
      ...
      {activeTab === "dashboard" && (
        <div className="ebi-grid">
          <section className="ebi-panel">
            <h3>Executive Summary</h3>
            <dl className="ebi-metrics">
