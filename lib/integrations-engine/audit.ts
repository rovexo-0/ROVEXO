...
export async function auditIntegrationsEngineAction(input: {
  actorId: string;
  module: string;
  component?: string;
  action: string;
  ...
  await auditSuperAdminAction({
    ...
    action: "integrations_engine.change",
    resourceType: "integrations_engine",
...
export function createIntegrationsEngineAuditEntry(input: {
