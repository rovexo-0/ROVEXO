...
export async function auditNotificationsEngineAction(input: {
  actorId: string;
  module: string;
  component?: string;
  action: string;
  ...
  await auditSuperAdminAction({
    ...
    action: "notifications_engine.change",
    resourceType: "notifications_engine",
...
export function createNotificationsEngineAuditEntry(input: {
