...
export async function auditPaymentsEngineAction(input: {
  actorId: string;
  module: string;
  component?: string;
  action: string;
  ...
  await auditSuperAdminAction({
    ...
    action: "payments_engine.change",
    resourceType: "payments_engine",
...
export function createPaymentsEngineAuditEntry(input: {
