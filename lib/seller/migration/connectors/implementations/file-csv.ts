import { BaseUniversalConnector } from "@/lib/seller/migration/connectors/base-connector";
import { loadSellerFileColumnMapping } from "@/lib/seller/migration/connectors/file/mapping-settings";
import {
  countCsvRows,
  listingsFromCsvContent,
  previewCsvContent,
  resolveFileContent,
} from "@/lib/seller/migration/connectors/file/csv-parser";
import type { ConnectorConnectInput, ConnectorDefinition } from "@/lib/seller/migration/connectors/types";
import type { MigrationConnectorInput } from "@/lib/seller/migration/engine/types";

export function createCsvConnector(definition: ConnectorDefinition): BaseUniversalConnector {
  return new BaseUniversalConnector(definition, {
    validateConfiguration: (input: ConnectorConnectInput) => {
      const content = input.fileContent ?? (input.settings?.fileContent as string | undefined);
      if (!input.fileName && !content) {
        return {
          valid: false,
          errors: [{ field: "fileName", message: "CSV file name or content is required." }],
        };
      }

      if (content) {
        const preview = previewCsvContent(content, 1);
        if (preview.rows.length === 0) {
          return {
            valid: false,
            errors: [{ field: "fileContent", message: "CSV file has no importable rows." }],
          };
        }
      }

      return { valid: true, errors: [] };
    },
    connect: async () => undefined,
    estimateTotal: async (input) => {
      const content = resolveFileContent(input.payload);
      if (content) return countCsvRows(content);
      return 0;
    },
    fetchListings: async (input: MigrationConnectorInput) => {
      const content = resolveFileContent(input.payload);
      if (!content) {
        throw new Error("CSV file content is missing from the migration job.");
      }

      const mapping = await loadSellerFileColumnMapping(input.sellerId, input.platform);
      return listingsFromCsvContent(
        content,
        input.platform,
        input.importMethod,
        input.offset,
        input.limit,
        mapping,
      );
    },
  });
}
