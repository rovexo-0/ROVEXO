...
export async function generateParcel2GoLabelForOrder(
  orderId: string,
  sellerId: string,
  parcelId?: string,
) {
  ...
  if (!quoteId || !isParcel2GoQuoteId(quoteId)) {
    return { ok: false as const, error: "No Parcel2Go quote selected for this order." };
  }
