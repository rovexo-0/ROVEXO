...
function buildServices(
  ...
  return [
    { id: "marketplace", label: "Marketplace", status: mapHealth(health.status), detail: health.status },
    { id: "database", label: "Database", status: mapHealth(checks.database.status), detail: checks.database.message },
    { id: "search", label: "Search", status: env.redis ? mapHealth(checks.redis.status) : "warning", detail: "Search indexing" },
    ...
    { id: "notifications", label: "Notifications", status: mapHealth(checks.email.status), detail: checks.email.message },
    ...
    { id: "email", label: "Email", status: mapHealth(checks.email.status) },
    { id: "push", label: "Push Notifications", status: "warning", detail: "Configure web push keys" },
    { id: "backup", label: "Backup", status: "online" },
    { id: "security", label: "Security", status: env.supabase ? "online" : "warning" },
    { id: "server", label: "Server", status: mapHealth(health.status) },
    { id: "api", label: "API", status: mapHealth(checks.api.status), detail: checks.api.message },
    { id: "queue", label: "Queue", status: mapHealth(checks.redis.status) },
    { id: "cache", label: "Cache", status: mapHealth(checks.redis.status) },
  ];
}
...
export async function getMissionControlSnapshot(): Promise<MissionControlSnapshot> {
  ...
  const platformHealth = mapHealth(dashboard.operations.health.status);
