#!/usr/bin/env node
/**
 * ROVEXO View Engine Master Spec v1.0 — apply SQL #5 RPC (optional when app commit path is live).
 * Usage:
 *   SUPABASE_DB_PASSWORD=... node scripts/apply-view-engine-master-spec.mjs
 * or:
 *   DATABASE_URL=postgres://... node scripts/apply-view-engine-master-spec.mjs
 */
...
const hasAdminRoleBlock = /v_role\s+in/i.test(body);
console.log("SELLER_BLOCK", hasSeller ? "PRESENT" : "MISSING");
console.log("ADMIN_ROLE_BLOCK", hasAdminRoleBlock ? "PRESENT" : "REMOVED");
await client.end();
process.exit(hasSeller && !hasAdminRoleBlock ? 0 : 1);
