/**
 * RUN #2 — UI Comparison Certification (DESIGN REVIEW ONLY)
 ...
 * Version B = horizontal page pad 9px (prototype)
...
html[data-run2-ui-compare="${pad}"] .account-canonical:has(.inbox-hub) .account-canonical-header__bar--titled,
html[data-run2-ui-compare="${pad}"] .account-canonical:has(.wallet-v2) .account-canonical-header__bar--titled {
...
html[data-run2-ui-compare="${pad}"] .cds-layout__content--account-canonical {
  padding-left: ${pad}px !important;
  padding-right: ${pad}px !important;
}
html[data-run2-ui-compare="${pad}"] .cds-layout__content--account-canonical:has(.wallet-v2) {
  padding-left: 0 !important;
  padding-right: 0 !important;
}
html[data-run2-ui-compare="${pad}"] .wallet-v2 {
  --wallet-pad-x: ${pad}px !important;
  padding-left: ${pad}px !important;
  padding-right: ${pad}px !important;
}
html[data-run2-ui-compare="${pad}"] .cds-layout__header,
html[data-run2-ui-compare="${pad}"] .cds-layout--account-canonical > .cds-layout__header {
  padding-left: ${pad}px !important;
  padding-right: ${pad}px !important;
}
html[data-run2-ui-compare="${pad}"] .account-settings-sticky-action,
html[data-run2-ui-compare="${pad}"] .settings-canonical,
html[data-run2-ui-compare="${pad}"] .ac-canonical,
html[data-run2-ui-compare="${pad}"] .orders-page {
  padding-left: ${pad}px !important;
  padding-right: ${pad}px !important;
}
/* Homepage / header / feeds */
html[data-run2-ui-compare="${pad}"] .rx4-home,
html[data-run2-ui-compare="${pad}"] .rovexo-page-home,
html[data-run2-ui-compare="${pad}"] main,
html[data-run2-ui-compare="${pad}"] .rx4-section,
html[data-run2-ui-compare="${pad}"] .rx4-feed,
html[data-run2-ui-compare="${pad}"] .rx4-rail,
html[data-run2-ui-compare="${pad}"] .canonical-homepage,
html[data-run2-ui-compare="${pad}"] .hp-section,
html[data-run2-ui-compare="${pad}"] .rx-fs,
html[data-run2-ui-compare="${pad}"] [data-home-section],
html[data-run2-ui-compare="${pad}"] .rvx-topbar,
html[data-run2-ui-compare="${pad}"] .rx-topbar,
html[data-run2-ui-compare="${pad}"] header[class*="home"],
html[data-run2-ui-compare="${pad}"] .homepage-header,
html[data-run2-ui-compare="${pad}"] [class*="Homepage"] {
  padding-left: ${pad}px !important;
  padding-right: ${pad}px !important;
  padding-inline: ${pad}px !important;
}
html[data-run2-ui-compare="${pad}"] .conv-hub,
html[data-run2-ui-compare="${pad}"] .conversation-hub {
  --conv-pad-x: ${pad}px !important;
  padding-left: ${pad}px !important;
  padding-right: ${pad}px !important;
}
...
async function applyPad(page: Page, pad: Pad) {
  await page.evaluate((p) => {
    document.documentElement.setAttribute("data-run2-ui-compare", String(p));
  }, pad);
...
function scorePage(
  ...
  const scores: Scores = {
    premiumLook: {
      a: 9.2,
      b: 8.4,
      reason: "24px preserves breathing room and premium edge inset; 9px feels denser / less ‘gallery’ margin.",
    },
    modernDesign: {
      a: 8.6,
      b: 9.0,
      reason: "9px trends toward edge-to-edge mobile feeds (marketplace density); 24px reads more editorial.",
    },
    readability: {
      a: 9.0,
      b: 8.5,
      reason: "Wider side gutters (24px) reduce edge fatigue; 9px slightly increases line length / edge proximity.",
    },
    spaceEfficiency: {
      a: 7.5,
      b: 9.3,
      reason: `Usable content width +${contentGain}px at 9px (${mA.contentWidth}→${mB.contentWidth}).`,
    },
    visualBalance: {
      a: 9.1,
      b: 8.3,
      reason: "24px mirrors locked Master Full Width symmetry; 9px shifts optical weight toward content mass.",
    },
    mobileUx: {
      a: 8.8,
      b: 8.9,
      reason: "Both mobile-first; 9px gains content, 24px gains touch-miss margin near bezels.",
    },
    oneHandUse: {
      a: 8.4,
      b: 8.7,
      reason: "9px pulls interactive rows slightly wider into thumb arc; 24px keeps controls more centered.",
    },
    consistency: {
      a: 10.0,
      b: 6.5,
      reason: "24px is current SSOT (Master Full Width Contract). 9px is prototype-only until Owner approval.",
    },
  };
  ...
  if (id === "homepage") {
    ...
    scores.spaceEfficiency.b = 9.5;
    scores.modernDesign.b = 9.2;
  }
  if (id === "messages-hub") {
...
async function main() {
  ensureDirs();
  console.log("RUN #2 UI Comparison — DESIGN REVIEW ONLY");
  ...
  for (const def of pages) {
    ...
    await applyPad(page, 9);
    await page.waitForTimeout(350);
    const measureB = await measurePad(page);
    const bPath = join(OUT, "b-9px", `${def.id}.png`);
    await page.screenshot({ path: bPath, fullPage: false });
    await sharp(bPath).png().toFile(join(OUT, "fullscreen", `${def.id}-b-9px.png`));
...
function writeReports(results: PageResultLike[]) {
  writeFileSync(join(OUT, "results.json"), JSON.stringify({
    run: "RUN #2 UI Comparison Certification",
    ...
    versionB: { label: "Comparison Prototype", padX: 9 },
    pages: results,
  ...
  const matrixMd = [
    "# Comparison Matrix — RUN #2",
    "",
    "| Page | Content Δ | Diff % | Overall A (24) | Overall B (9) | Winner (score) |",
    "|------|-----------|--------|----------------|---------------|----------------|",
    ...results.map((r) => {
      const a = r.scores.overall.a;
      const b = r.scores.overall.b;
      const win = a === b ? "TIE" : a > b ? "A 24px" : "B 9px";
  ...
  const designMd = [
    "# Design Review Report — RUN #2",
    "",
    "Scope: horizontal page padding only (24px vs 9px). Typography, colours, buttons, cards, icons, radii, header, bottom nav, animations unchanged.",
    "",
    ...results.flatMap((r) => [
      `## ${r.label}`,
      "",
      "- Visible content area: B gains ~30px usable width on this viewport.",
      "- Card alignment: same grid; outer gutter shrinks.",
      "- Text readability: A safer near-edge; B denser.",
      "- Button positioning: unchanged sizes; slightly wider hit area span at 9px.",
      "- Thumb reach: marginal gain for edge controls at 9px.",
  ...
  const recMd = [
    ...
    "3. 9px improves space efficiency (~+30px content) and modern density — valid product trade-off, but not SSOT without Owner approval.",
    "",
    "### If Owner prefers 9px later",
    "- Requires explicit Owner approval + Master Full Width Contract update.",
    "- Must re-certify entire platform (not Homepage alone).",
    "- This RUN #2 package is evidence only — no production change applied.",
    "",
...
<section class="page" id="${r.id}">
  ...
  <h3>① Current (24px) · ② Prototype (9px) · ③ Side-by-side</h3>
  <div class="grid2">
    <figure><img src="a-24px/${r.id}.png" alt="A 24px"/><figcaption>A · 24px</figcaption></figure>
    <figure><img src="b-9px/${r.id}.png" alt="B 9px"/><figcaption>B · 9px</figcaption></figure>
  </div>
  ...
  <div class="split" data-split>
    <img class="split-b" src="b-9px/${r.id}.png" alt="B"/>
    ...
    <div class="split-labels"><span>A 24px</span><span>B 9px</span></div>
  </div>

  <h3>⑥ Full Screen Preview</h3>
  <div class="grid2">
    <figure><img src="fullscreen/${r.id}-a-24px.png" alt="Full A"/><figcaption>Full A</figcaption></figure>
    <figure><img src="fullscreen/${r.id}-b-9px.png" alt="Full B"/><figcaption>Full B</figcaption></figure>
  </div>
  ...
  <h3>Scores</h3>
  <table><thead><tr><th>Criterion</th><th>A 24</th><th>B 9</th><th>Reasoning</th></tr></thead><tbody>${rows}</tbody></table>
</section>`;
    })
    .join("\n");
...
<title>ROVEXO RUN #2 — UI Comparison Certification</title>
<style>
...
<body>
<header>
  <h1>ROVEXO v1.0 — RUN #2 UI Comparison Certification</h1>
  <div class="banner">DESIGN REVIEW ONLY · No production / SSOT / commit / DB changes · iPhone 17 Pro Max 440×956 · 6.9"</div>
  <div style="margin-top:8px;font-size:13px;color:var(--muted)">A = Canonical 24px · B = Prototype 9px · Only horizontal page padding differs</div>
  <nav>${results.map((r) => `<a href="#${r.id}">${escape(r.label)}</a>`).join("")}</nav>
</header>
<main>
  <section class="page">
    <h2>Recommendation (preview)</h2>
    <p>Keep <strong>24px</strong> as locked SSOT. 9px gains ~30px usable width and density scores, but loses premium margin / consistency with Master Full Width Contract. Owner approval required before any global change.</p>
  </section>
  ${cards}
</main>
