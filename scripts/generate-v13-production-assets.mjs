/**
 * ROVEXO v1.3 — Final production asset pipeline (Prompt 013)
 * Run: node scripts/generate-v13-production-assets.mjs
 */
...
async function exportHeroCampaign(id, svg) {
  ...
  const blurBuffer = await sharp(sourceBuffer).resize(32, 18, { fit: "cover" }).webp({ quality: 20 }).toBuffer();
  console.log(`✓ hero ${id}`);
  return blurBuffer;
}
...
async function main() {
  ...
  console.log(`\n✓ Generated ${allIcons.length} category icons + ${HERO_CAMPAIGN_IDS.length} hero campaigns (v1.3 production)`);
}
