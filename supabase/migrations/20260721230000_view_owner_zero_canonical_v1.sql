-- ROVEXO v1.0 — OWNER = 0 CANONICAL (does not modify SQL #1 / #2 / #3 files)
-- Restores listing-seller forever-block on record_unique_product_view.
-- Admin / super_admin = 0 · Unpublished = 0 · Unique viewer 24h · Anti-spam 60/hour.
-- App layer also enforces OWNER = 0 before RPC (lib/views/record-product-view.ts).
...
begin
  ...
  -- OWNER = 0 — seller never inflates own views (1…10000 opens → +0)
...
comment on function public.record_unique_product_view(text, text, uuid) is
  'ROVEXO v1.0 — OWNER=0 · admin/super_admin=0 · unpublished=0 · unique viewer 24h · anti-spam 60/hour';
