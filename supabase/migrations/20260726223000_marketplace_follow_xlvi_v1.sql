-- ROVEXO Absolute Blood Code XLVI — Marketplace Follow + counters
-- Marketplace follow ONLY (not social media). Extends architecture in place.
-- Does NOT recreate dropped seller_follows — new canonical table: user_follows.
...
-- Review window: track delivery-kept countdown (4 days) — Blood Code XLVI
alter table public.orders
  add column if not exists review_window_opens_at timestamptz,
...
comment on table public.user_follows is
  'Blood Code XLVI marketplace Follow — notifications for listings/price/reviews only. Not social media.';
