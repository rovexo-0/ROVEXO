...
describe("Inbox Hub Master Implementation — UI LOCK", () => {
  it("marks Inbox Hub as permanently locked", () => {
    expect(INBOX_HUB_CANONICAL_FROZEN).toBe(true);
    expect(INBOX_HUB_CANONICAL_STATUS).toBe("PERMANENTLY LOCKED");
  ...
  it("locks DOM freeze markers on hub surfaces", () => {
    ...
    expect(inbox).toContain("inbox-hub__thumb");
    expect(inbox).not.toContain("inbox-hub__avatar");
    expect(inbox).not.toContain("inbox-hub__avatar-face");
    ...
    expect(conversation).toContain('data-conversation-hub-ui="v2-canonical"');
    ...
    expect(conversationCss).toContain("--conv-composer-h: 48px");
    ...
    expect(inboxCss).toContain("--inbox-thumb: 56px");
    expect(inboxCss).toContain("--inbox-tab-list-gap: 8px");
    expect(inboxCss).toContain("--inbox-tab-underline-gap: 6px");
    expect(inboxCss).toContain("grid-template-columns");
    expect(inboxCss).toContain(":has(.inbox-hub)");
    expect(inboxCss).toContain("padding-top: 12px !important");
  });
  ...
  it("locks conversation status step ids and v1.1 visual constants", () => {
    ...
    expect(INBOX_HUB_VISUAL_LOCK.cardPaddingPx).toBe(0);
    ...
    expect(INBOX_HUB_VISUAL_LOCK.purple).toBe("#9333ea");
    expect(INBOX_HUB_VISUAL_LOCK.purpleGradient).toContain("#a855f7");
  });
  ...
  it("preserves offer, tracking, dispute, review, and MES action-bar contracts in-thread", () => {
    const hub = readSource("features/inbox/components/ConversationHub.tsx");
    const actionBar = readSource("features/inbox/components/TransactionActionBar.tsx");
    const viewSource = readSource("lib/inbox/conversation-view.ts");
    expect(hub).toContain("/api/offers/");
    expect(actionBar).toContain('data-transaction-action-bar="v1.1"');
    expect(hub).toContain("<TransactionActionBar");
    expect(hub).toContain("/api/shipping/labels?orderId=");
    expect(hub).toContain('data-conversation-realtime={demoMode ? "demo" : "live"}');
    expect(hub).not.toContain("View Label");
    expect(hub).toContain("productCardStatus");
    expect(hub).toContain('className="conv-hub__system-view"');
    expect(hub).toContain('"view_label"');
    expect(viewSource).not.toContain("Waiting for first carrier scan");
    expect(viewSource).not.toContain("system-waiting-scan");
    expect(viewSource).not.toContain("system-payment-");
    expect(actionBar).toContain('name="shipping"');
    expect(hub).toContain("SafeImage");
    expect(hub).toContain("<OrderReviewCard");
    expect(hub).toContain("setReviewOpen(true)");
    expect(hub).not.toContain("window.prompt");
    expect(hub).toContain("download_label");
    expect(viewSource).not.toContain('label: "Get Shipping Label"');
    ...
    expect(hub).toContain("matchingOrders.length === 0");
    expect(hub).toContain("track_parcel");
