...
describe("canonical product action bar", () => {
  it("locks Buy Now + Make Offer only — cart removed forever", () => {
    expect(PRODUCT_ACTION_BUTTONS.map((b) => b.id)).toEqual(["buy_now", "make_offer"]);
    expect(PRODUCT_ACTION_BAR_COPY.buyNow).toBe("Buy Now");
    expect(PRODUCT_ACTION_BAR_COPY.buyNowLoading).toBe("Loading...");
  ...
  it("uses responsive two-column grid without fixed button widths", () => {
    ...
    expect(PRODUCT_ACTION_BAR_LAYOUT.columnCount).toBe(2);
  ...
  it("locks production UI tokens", () => {
    ...
    expect(PRODUCT_ACTION_BAR_VERSION).toBe("v1.1-canonical-freeze");
    ...
    expect(hook).not.toContain("flushHubActionQueue");
    expect(hook).not.toContain("cart-engine");
  });
});
